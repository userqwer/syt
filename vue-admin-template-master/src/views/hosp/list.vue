<template>
  <div class="app-container">
    ...
    <!-- 分页组件 -->
    <!-- :page-sizes="[5, 10, 20, 30, 40, 50, 100]" 每页显示几条（5？10？....） -->
    <!-- 页码变了就调这个fetchData方法重新获取数据 -->
    <!-- 每页条数变了就调这个changeSize方法，显示每页多少条 -->
    <el-pagination
      :current-page="page"
      :total="total"
      :page-size="limit"
...
export default {
  data() {
    return {
      listLoading: true, // 数据是否正在加载（在显示列表中转/加载的图标）
      list: null, // 医院列表数据集合
      total: 0, // 数据库中的总记录数
      page: 1, // 默认页码
      limit: 10, // 每页记录数
  ...
  created() {
    ...
    this.fetchData();
    //调用查询所有省的方法
    this.findAllProvince(); //第一次进入页面需要把所有的省份显示
  },
  methods: {
    //更新医院上线状态
