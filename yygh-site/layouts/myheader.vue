<template>
  <div class="header-container">
    <div class="wrapper">
      ...
      <!-- 右侧 
      v-if="name == ''"：如果从cookie中获取不到name，则显示登录/注册
      v-if="name != ''"：如果name不等于空，则显示用户的名称和其他部分-->
    ...
    <!-- 登录弹出层 
    dialogUserFormVisible：如果该值是true就弹窗，如果是false就不弹窗-->
    <el-dialog
    ...
    >
      <div class="container">
        <!-- 手机登录 #start 
        labelTips：手机号码
        v-model="dialogAtrr.inputValue"：与手机号码输入框进行双向绑定-->
...
const defaultDialogAtrr = {
  showLoginType: "phone", // 控制手机登录与微信登录切换 默认是phone登录
...
export default {
  data() {
    return {
      ...
      // 弹出层相关属性
      // 赋值操作，相当于dialogAtrr里就有defaultDialogAtrr里的属性了
  ...
  mounted() {
    ...
    // 监听登录事件(名字是 loginDialogEvent)
    loginEvent.$on("loginDialogEvent", function () {
      //当别人调用loginDialogEvent这个监听事件后，下面的代码就会执行，然后触发登录/注册的方法@click="showLogin()"
      // 格式是：全局登录事件对象.$emit('事件名')：loginEvent.$emit('loginDialogEvent')
      document.getElementById("loginDialog").click();
    });
    // 在别的页面就可以触发事件对象，并显示登录弹窗：
    ...
    document.body.appendChild(script); //把js片段放入body里

    // 微信登录回调处理
    let self = this; //this代表当前页面
  ...
  methods: {
    loginCallback(name, token, openid) {
      ...
      if (openid != null) {
        ...
        this.showLogin(); //如果openid不等于空，表示是第一次通过扫码的方式登录，显示登陆弹窗，输入手机号，从而绑定手机，
      } else {
        this.setCookies(name, token); //把name和token放到cookie里面
      }
    },
    // 绑定登录或获取验证码按钮
    btnClick() {
      ...
      if (this.dialogAtrr.loginBtn == "获取验证码") {
        this.userInfo.phone = this.dialogAtrr.inputValue; //获取到手机号
    ...
    setCookies(name, token) {
      cookie.set("token", token, { domain: "localhost" }); //domain: "localhost"：范围是localhost
      cookie.set("name", name, { domain: "localhost" });
      window.location.reload(); //最后把页面重新加载
    },
    ...
    timeDown() {
      ...
      this.clearSmsTime = setInterval(() => {
        --this.dialogAtrr.second; //每隔一秒减一（59,58,57,56,55.....）
        if (this.dialogAtrr.second < 1) {
          //如果秒数小于1，则清除定时器效果，并回到初始值状态
          clearInterval(this.clearSmsTime);
